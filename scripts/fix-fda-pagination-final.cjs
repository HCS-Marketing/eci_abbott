...
async function main() {
  ...
  console.log('Using $executeRawUnsafe with (id, fecha, categoria) join\n')
  ...
  for (const { d } of dates) {
    ...
    try {
      ...
      if (done % 10 === 0 || done === 1) {
        ...
        console.log(`  ${done}/${dates.length} (${ds}) [${elapsed}s, ~${eta}s left, ${affected} rows]`)
      }
    } catch(e) {
      console.error(`ERROR on ${ds}: ${e.message}`)
